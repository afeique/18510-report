...
\begin{figure}[ht]
  ...
  \caption{\em Condenser microphone diaphragm system~\supercite{torio}}
...
\begin{figure}[ht]
	...
	\end{tabular}
	\caption{\em Symbols used in membrane deflection analysis}
	\label{tab:MembraneSymbols}
\end{figure}

\subsection{Membrane Deflection}

When sound pressure impinges upon the membrane, it oscillates with the pressure according to its mechanical properties. Fig. \ref{fig:MembraneDeflection} shows modeled parabolic deflection in cylindrical coordinates. The modeled parabolic deflection assumes sound pressure is uniformly distributed across the face of the membrane. This produces purely vertical membrane oscillation along the $z$-axis in cylindrical coordinates, with the apex deflection $w_0$ occurring at the center of the membrane owing to the moment arm at that point.

Setting residual stress $\sigma_0=0$, we can solve for the apex deflection using the relation\supercite{schomburg}:
...
\begin{figure}[ht]
	...
	\end{tabular}
	\caption{\em Symbols used in apex deflection relationship to $\Delta p$}
	\label{tab:ApexSymbols}
\end{figure}

The apex deflection $w_0$ can then be used to calculate the modeled parabolic deflection at any radius $r$ using the relation:
...
Let the air-gap between the membrane and back-plate be denoted by $h_0$. If we treat the face of the membrane as a collection of  cylindrical area differentials with $dA = r\,dr\,d\theta$ we can find the total capacitance under deflection by integrating the differential capacitances $dC$:
